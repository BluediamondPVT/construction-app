...
export default function DesignDashboardOverview() {
  const metrics = [
    { name: "Active Blueprints", value: "18", icon: Layers, color: "text-blue-400", change: "AutoCAD & Revit files" },
    { name: "Pending BOQs", value: "3", icon: FileCheck, color: "text-orange-400", change: "Requested by CRM team" },
    { name: "Design Revisions", value: "4", icon: PenTool, color: "text-purple-400", change: "Needs architect approval" },
    { name: "Approved Standards", value: "100%", icon: Ruler, color: "text-emerald-400", change: "NFPA/NBC compliant" },
  ];
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center">
            <PenTool className="mr-3 h-8 w-8 text-blue-500 drop-shadow-lg" />
            Design & Technical Hub
          </h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium max-w-2xl">
            Maintain version control for drawings, manage technical documents, and process BOQ requests from marketing.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl p-10 md:p-12 border border-slate-200/50 dark:border-white/10 shadow-sm text-center min-h-[350px] flex flex-col justify-center items-center">
        <div className="h-20 w-20 bg-blue-500/10 rounded-full flex items-center justify-center mb-6 border border-blue-500/20 shadow-lg shadow-blue-500/10">
          <Layers className="h-10 w-10 text-blue-500" strokeWidth={1}/>
        </div>
        <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white mb-2">Blueprint Vault System</h2>
        <p className="text-slate-500 dark:text-slate-400 max-w-lg mb-8 font-medium">
          The version control system for drawings (Draft 1, Final As-Built) and the BOQ upload portal are currently being integrated.
        </p>
        <Link href="/dashboard/design/blueprints" className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold rounded-xl shadow-md transition-all">
          Access Blueprints <ArrowRight className="h-4 w-4 ml-2" />
        </Link>
      </div>
    </div>
  );
