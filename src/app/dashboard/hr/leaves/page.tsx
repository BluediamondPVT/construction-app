                        ...
                        <div key={leave._id} className="bg-white dark:bg-slate-800/50 backdrop-blur-sm p-5 rounded-2xl border border-yellow-200 dark:border-yellow-700/50 shadow-sm flex flex-col justify-between">
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-2 mb-3 mt-1">
                                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/50 px-2 py-1 rounded">
                                        {leave.type} Leave • {new Date(leave.date).toLocaleDateString()}
                                    </p>
                                    {leave.isPaidLeaveQuotaUsed ? (
                                        <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-300 text-[11px] font-bold">
                                            🔴 Quota Used ({leave.approvedLeavesThisMonth || 1}/1) → LWP
                                        </span>
                                    ) : (
                                        <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-[11px] font-bold">
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-50 dark:bg-slate-800/80 border-b dark:border-slate-700">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Pay Impact</th>
                                    ...
                                    <tr key={leave._id} className="border-b dark:border-slate-700/50 last:border-0 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border ${
                                                ...
                                                {leave.payImpactText || (leave.isPaidLeaveQuotaUsed ? "LWP" : "Paid Leave (1 Quota)")}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
